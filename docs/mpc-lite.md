...
- **Deterministic client share derivation** from passkey PRF (`PRF.first`) (no VRF WASM / no VRF proofs).
- **Wallet-origin boundary** (cross-origin wallet-iframe / extension) so app-origin code can’t read PRF outputs or derived secrets.
- Minimal **SecureConfirm** UX in the wallet origin (approve intents + satisfy user-activation requirements for WebAuthn).
...
- VRF worker WASM (`wasm_vrf_worker`) and all VRF proof/session flows.
- Shamir 3-pass relay wrapping, PRF.second recovery paths, email recovery flows.
- Local signer mode for day-to-day signing (no `local_near_sk_v3` vault lifecycle).
- App-origin UI frameworks/components (React/Lit) bundled into the app; keep wallet-origin UI minimal and purpose-built.
...
- Avoid pulling in VRF WASM and VRF proof scaffolding.

### Proposed architecture
- Wallet-origin code:
  - Owns WebAuthn calls (challenge/response + PRF extension).
  - Shows SecureConfirm UI (intent review + explicit approval).
- One module worker (wallet origin): `mpc-signer.worker.js`
  - Derives client threshold signing share from `PRF.first` (during `connectPasskey`)
  - Holds the share (or `PRF.first`) in memory until session expiry (TTL/uses)
  - Coordinates with relay endpoints for `/threshold-ed25519/*`
