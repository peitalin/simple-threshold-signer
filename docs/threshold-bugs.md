...
The client “requests” `ttlMs` and `remainingUses` by embedding them in the `sessionPolicyDigest32` (the WebAuthn challenge digest for the session policy).
The server cannot silently change (“clamp”) those values without invalidating the digest.

Current behavior:
- The client helper and the relayer both **clamp** `ttlMs` / `remainingUses` to conservative maxes **before** computing/verifying the policy digest.
- If a client supplies values above the caps without applying the same clamp, verification will fail (challenge digest mismatch).

## Bug: threshold sessions exhausted unexpectedly (stable `sessionId` / 1-use policies)

### Symptoms
- Threshold signing works once, then subsequent signing attempts fail early (often at `POST /threshold-ed25519/session` or `POST /threshold-ed25519/authorize`) with `unauthorized` / exhausted-session errors.
- Multi-tx signing can exhaust a session immediately if it is minted with `remainingUses=1`.

### Diagnosis
- The client was reusing a stable `sessionId` when building the threshold session policy (e.g. a worker session id), causing:
  - the relayer to treat multiple mints as the same session, and
  - remaining-uses to be decremented to zero quickly.
- Some call sites minted threshold sessions with `remainingUses` too low for the number of signatures required.

### Fix
- The client now:
  - mints fresh threshold sessions (do not reuse a stable `sessionId`), and
  - requests `remainingUses >= usesNeeded` (defaulting to `signingSessionDefaults.remainingUses` when present).
