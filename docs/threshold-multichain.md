...
- Keep the backend “what to sign” agnostic:
  - backend signs canonical **32-byte** digests (v1), not chain transactions.
...
- Implementing multi-party (t-of-n) across _independent_ trust domains in v1 (we keep 2-party externally: client + logical relayer).
...
- transcript binding invariants: `(schemeId, relayerKeyId, digest32, participantIds, thresholdExpiresAtMs)` + replay protection
...
- build canonical `digest32` bytes and a user-reviewable UI model
...
- **App session** (passkey / SSO):
  ...
  - Does _not_ automatically grant signing rights.
- **Threshold signing session** (step-up):
  - Grants access to `/threshold-*/authorize` and `/threshold-*/sign/*`.
  - Binds to a specific signing scope (scheme/kind, relayerKeyId, participantIds, rpId, thresholdExpiresAtMs; remaining uses are tracked server-side by `sessionId`).
...
- Use the same WebAuthn verification machinery, but mint a **threshold signing session** (`/threshold-*/session`) scoped to `(scheme/kind, relayerKeyId, rpId, participantIds, thresholdExpiresAtMs)`.
...
- validates transcript binding invariants (must bind `(schemeId, relayerKeyId, digest32, participantIds, thresholdExpiresAtMs)` and reject replays/mismatches)
...
Chain adapters stay client-side only: they produce `digest32` bytes + UI model and finalize signatures into chain-specific outputs.
...
- `digest32` (32 bytes) and metadata
...
- `rpId`, `userId`
- `remainingUses`, `thresholdExpiresAtMs`
...
- Protocol: NEAR `threshold-signatures` (Cait-Sith-derived OT-based ECDSA).
- Client-facing signature output: `r(32) || s(32) || recId(1)` (wallet derives `yParity/v`).
- Latency: use presigning/preprocessing (presignature pool) to keep the online signing path fast enough for wallet UX.
...
- Implement relayer-side signer module:
  - resolves relayer share material for a given `relayerKeyId`
...
- Implement transcript validation rules:
  - bind signature sessions to `(schemeId, relayerKeyId, digest32, participantIds, thresholdExpiresAtMs)`
...
- threshold session JWT binds (current v1 claim set):
  - `kind` (`threshold_ed25519_session_v1` | `threshold_ecdsa_session_v1`) = scheme/version discriminator
  - `sub` (= `userId`), `sessionId`, `relayerKeyId`, `rpId`, `participantIds`, `thresholdExpiresAtMs`
...
- [x] Implement relay-side presignature pool management (reserve, consume, discard) and wire it into `/threshold-ecdsa/sign/*` (Redis/Upstash/Postgres/DO + in-memory for local dev).
...
- [ ] If cookie-based sessions are used broadly, add CSRF protections for state-changing endpoints and standardize cookie flags (`Secure`, `HttpOnly`, `SameSite`).

Later (optional / product):

- [ ] Add redirect-based Google OIDC (`/auth/google/start`, `/auth/google/callback`) for deployments that do not use client-side `id_token`.
- [ ] Add account-merge UX for identity-link conflicts (subject already linked to another user).
- [ ] Implement relayer-fleet cosigning for threshold-ECDSA (coordinator + internal cosigners) once needed.
...
2. **Keep derived shares + encode them as Shamir shares for a fixed 2-party signer set** (fastest path for 2P):
   - Treat derived shares as the _linearized_ shares the protocol wants after Lagrange weighting.
...
- For a 2-party **wallet (client) ↔ relayer** signer set, both parties must hold a synchronized pool of presignature _shares_; refills typically happen at login/warm-session time (when the wallet origin is online).
...
- Concurrency: pool operations must be atomic per key (Redis/Upstash `EVAL` reserve script, DO transactions, or SQL transactions) to prevent double-spend of a presignature.
- Operational requirement: Redis/Upstash deployments must allow Lua scripting (`EVAL`) for atomic reserve; the relay now fails fast when scripting is unavailable (no non-atomic fallback).
