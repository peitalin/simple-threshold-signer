...
> Deprecated: this document captures the **legacy VRF / on-chain verifier** design. The current VRF-free lite/threshold signer design lives in:
> - `docs/lite-signer-refactor/plan.md`
> - `docs/lite-signer-refactor/sdk-contract.md`
...
> Status: legacy reference only; may not match the current codebase.
...
- Can verify WebAuthn assertions (contract-free; relay-stored authenticators) and optionally issue a session (JWT/cookie).

Relevant code:
- `sdk/src/server/router/express-adaptor.ts`
- `sdk/src/server/delegateAction/index.ts`
- `sdk/src/core/rpcCalls.ts` (`createWebAuthnLoginOptions`, `verifyWebAuthnLogin`)
...
- Routes (default):
  - `POST /login/options` (mint a server-side WebAuthn challenge)
  - `POST /login/verify` (verify assertion + issue session)
- Behavior: relayer verifies authentication (contract-free; relay-stored authenticators + counter persistence) and issues a session credential:
...
- It must authenticate requests (standard WebAuthn; e.g., relayer sessions via `POST /login/options` → `POST /login/verify`, or threshold sessions via `POST /threshold-ed25519/session`).
...
Use a relayer session (`/login/options` → `/login/verify`) or a threshold session token (`POST /threshold-ed25519/session`) to gate `/threshold-ed25519/authorize`:
