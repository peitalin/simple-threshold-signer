...
test.describe('confirm-ui inline confirmer', () => {
  ...
  test('modal confirm resolves with confirmed=true', async ({ page }) => {
    const result = await page.evaluate(async ({ securityContext, summary, waitForSource, paths }) => {
      ...
      const { awaitConfirmUIDecision } = mod as typeof import('@/core/signing/secureConfirm/ui/lit-components/confirm-ui');
      ...
      return { confirmed };
    }, { securityContext: SECURITY_CONTEXT, summary: SUMMARY, waitForSource: WAIT_FOR_SOURCE, paths: IMPORT_PATHS });
