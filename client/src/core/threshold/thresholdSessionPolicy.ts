...
export async function computeThresholdEcdsaSessionPolicyDigest32(policy: ThresholdEcdsaSessionPolicy): Promise<string> {
  const json = alphabetizeStringify(policy);
  const bytes = await sha256BytesUtf8(json);
  return base64UrlEncode(bytes);
}
...
export async function buildThresholdEcdsaSessionPolicy(params: {
  userId: string;
  rpId: string;
  relayerKeyId: string;
  participantIds?: number[];
  ...
  const participantIds = normalizeThresholdEd25519ParticipantIds(params.participantIds);
  const policy: ThresholdEcdsaSessionPolicy = {
    version: THRESHOLD_SESSION_POLICY_VERSION,
  ...
  const sessionPolicyDigest32 = await computeThresholdEcdsaSessionPolicyDigest32(policy);
  return { policy, policyJson: JSON.stringify(policy), sessionPolicyDigest32 };
}
