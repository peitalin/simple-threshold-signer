...
export class PasskeyNearKeysDBManager {
  ...
  async storeKeyMaterial(data: PasskeyNearKeyMaterial): Promise<void> {
    ...
    if (data.kind === 'local_near_sk_v3') {
      if (!data.wrapKeySalt) {
        throw new Error('PasskeyNearKeysDB: Missing wrapKeySalt for local_near_sk_v3');
      }
      if (!data.encryptedSk) {
  ...
  async getKeyMaterial(
    ...
    const sanitize = (rec: any): PasskeyNearKeyMaterial | null => {
      ...
      if (!kind) return null;
      if (!rec?.publicKey || typeof rec?.timestamp !== 'number') return null;

      if (kind === 'local_near_sk_v3') {
        if (!rec?.wrapKeySalt) return null;
