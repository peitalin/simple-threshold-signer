...
import { type ValidationResult, validateNearAccountId } from '../../../../shared/src/utils/validation';
...
const DB_CONFIG: PasskeyClientDBConfig = {
  ...
  dbVersion: 16, // v16: add recoveryEmails store (was missing in v15 build)
  userStore: 'users',
...
const LAST_USER_APP_STATE_KEY = 'lastUserAccountId' as const;
...
function parseLastUserAccountIdState(raw: unknown): LastUserAccountIdState | null {
...
export class PasskeyClientDBManager {
   ...
   * `lastUserAccountId::<scope>` and read with scoped-first + legacy fallback.
   *
  ...
  private getScopedLastUserAppStateKey(scope: string | null = this.lastUserScope): string | null {
    return makeScopedAppStateKey(LAST_USER_APP_STATE_KEY, scope);
  }

  private async getLastUserAccountIdState(): Promise<LastUserAccountIdState | null> {
    const scopedKey = this.getScopedLastUserAppStateKey();
    if (scopedKey) {
      const scopedRaw = await this.getAppState<unknown>(scopedKey).catch(() => undefined);
      const scoped = parseLastUserAccountIdState(scopedRaw);
      if (scoped) return scoped;
    }

    const legacyRaw = await this.getAppState<unknown>(LAST_USER_APP_STATE_KEY).catch(() => undefined);
    return parseLastUserAccountIdState(legacyRaw);
  }

  private async setLastUserAccountIdState(state: LastUserAccountIdState | null): Promise<void> {
    // Always write legacy key for backwards compatibility.
    await this.setAppState(LAST_USER_APP_STATE_KEY, state);
    // Also write scoped key when scope is configured (wallet-iframe multi-app).
    const scopedKey = this.getScopedLastUserAppStateKey();
    if (scopedKey) {
  ...
  private async clearLastUserAccountIdIfMatches(accountId: AccountId): Promise<void> {
    try {
      const legacy = parseLastUserAccountIdState(await this.getAppState<unknown>(LAST_USER_APP_STATE_KEY));
      if (legacy && legacy.accountId === accountId) {
        await this.setAppState(LAST_USER_APP_STATE_KEY, null);
      }
    } catch {}
    ...
    if (!scopedKey) return;
    try {
      const scoped = parseLastUserAccountIdState(await this.getAppState<unknown>(scopedKey));
      if (scoped && scoped.accountId === accountId) {
        await this.setAppState(scopedKey, null);
      }
    } catch {}
  }
  ...
  /**
   * Clear the last-user pointer (logout / no active account selected).
   */
  ...
  async rollbackUserRegistration(nearAccountId: AccountId): Promise<void> {
    ...
    await this.atomicOperation(async (db) => {
      ...
      await this.clearLastUserAccountIdIfMatches(toAccountId(nearAccountId));
