...
- **Files:** `signingWorkerManager/index.ts`, `near-signer.worker.ts`
...
### Root Cause
- Manager sends health check ping: `worker!.postMessage({ type: 'WORKER_PING' })` at `signingWorkerManager/index.ts:164`
