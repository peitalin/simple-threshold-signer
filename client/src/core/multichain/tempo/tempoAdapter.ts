import type { ChainAdapter, SigningIntent, SignatureBytes } from '../types';
...
export class TempoAdapter implements ChainAdapter<TempoSigningRequest, unknown, TempoSignedResult> {
  ...
  async buildIntent(request: TempoSigningRequest): Promise<SigningIntent<unknown, TempoSignedResult>> {
    ...
    if (request.kind === 'eip1559') {
      const txHash = await computeEip1559TxHashWasm(request.tx);
      ...
      return {
        ...
        finalize: async (sigs: SignatureBytes[]) => {
          ...
          const raw = await encodeEip1559SignedTxWasm({ tx: request.tx, yParity, r, s });
    ...
    if (request.kind === 'tempoTransaction') {
      const senderHash = await computeTempoSenderHashWasm(request.tx);
      ...
      return {
        ...
        signRequests: [
          request.senderSignatureAlgorithm === 'webauthn-p256'
            ? {
                kind: 'webauthn',
        ...
        finalize: async (sigs: SignatureBytes[]) => {
          ...
          const raw = await encodeTempoSignedTxWasm({ tx: request.tx, senderSignature });
