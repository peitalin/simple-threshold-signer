...
import type { SecureConfirmWorkerManagerContext } from '../SecureConfirmWorkerManager';
import type { TransactionSummary } from '../SecureConfirmWorkerManager/confirmTxFlow/types';
...
function resolveTheme(_ctx: SecureConfirmWorkerManagerContext, requested?: ThemeName): ThemeName {
...
export async function mountConfirmUI({
  ...
  // 'none' mode should never request a UI mount; callers handle this.
...
function applyHostElementProps(el: HostTxConfirmerElement, props?: ConfirmUIUpdate): void {
  ...
  if (props.securityContext != null) el.securityContext = props.securityContext;
...
function mountHostElement({
  ...
  if (securityContext) el.securityContext = securityContext;
