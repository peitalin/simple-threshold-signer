...
function ensureTatchiPasskey(): void {
  ...
  if (!tatchiPasskey) {
    ...
    // Warm critical resources (Signer/SecureConfirm workers, IndexedDB) on the wallet origin.
...
async function onPortMessage(e: MessageEvent<ParentToChildEnvelope>) {
  ...
  if (req.type === 'PM_SET_CONFIG') {
    ...
    walletConfigs = {
      ...
      relayerAccount: payload?.relayerAccount || walletConfigs?.relayerAccount || payload?.contractId || walletConfigs?.contractId || '',
