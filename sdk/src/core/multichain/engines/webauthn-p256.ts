import { concatBytes } from '../evm/bytes';
...
export class WebAuthnP256Engine implements SignerEngine {
  readonly algorithm = 'webauthn-p256' as const;

  async sign(req: SignRequest, keyRef: KeyRef): Promise<SignatureBytes> {
    if (req.kind !== 'webauthn' || req.algorithm !== 'webauthn-p256') {
    ...
    if (keyRef.type !== 'webauthn-p256') {
      throw new Error('[WebAuthnP256Engine] keyRef must be webauthn-p256');
    }
