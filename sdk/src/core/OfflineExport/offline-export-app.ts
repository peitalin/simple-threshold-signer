...
async function registerServiceWorker(): Promise<void> {
  ...
  // Do not block UI on service worker installation/readiness; offline export must
  // render even if SW registration fails (e.g. during dev proxying).
  try {
    await Promise.race([
...
async function waitForServiceWorkerReady(timeoutMs = 2000): Promise<void> {
  if (!('serviceWorker' in navigator)) return;
  try {
    await Promise.race([
      navigator.serviceWorker.ready.then(() => void 0).catch(() => void 0),
...
async function prewarmSdkAssets(): Promise<void> {
  ...
  try {
    ...
    const priority = new Set<string>([
      ...
      '/offline-export/workers/web3authn-secure-confirm.worker.js',
      '/offline-export/workers/wasm_signer_worker_bg.wasm',
      '/sdk/workers/web3authn-signer.worker.js',
      '/sdk/workers/web3authn-secure-confirm.worker.js',
...
function renderShell(message: string, canExport = false): {
  root: HTMLDivElement;
  ...
  return { root, messageEl: p, button: btn };
}

async function main(): Promise<void> {
  // Kick off SW boot in the background. The SW only improves offline reliability;
  // the route should still render its shell even if SW registration/ready fails.
  const swBoot = (async () => {
  ...
  // Do not block first paint — fire-and-forget (and only when online).
  void swBoot.then(() => prewarmSdkAssets())

  // Render a minimal shell immediately so the route is usable even if IndexedDB
  // access is slow/unavailable in the current browser context.
  const shell = renderShell('Preparing offline export…', false);
  const btn = shell.button;
  ...
  try {
    ...
    ;(window as any).__W3A_SECURE_CONFIRM_WORKER_URL__ = '/offline-export/workers/web3authn-secure-confirm.worker.js'
    ...
    // Detect last user (same origin IndexedDB). Use short timeouts so this
    // route still renders in environments where IndexedDB is blocked/hangs.
    const withTimeout = async <T>(p: Promise<T>, fallback: T, timeoutMs = 1500): Promise<T> => {
      try {
    ...
    const last = await withTimeout(IndexedDBManager.clientDB.getLastUser(), null, 1500);
    const users = await withTimeout(IndexedDBManager.clientDB.getAllUsers(), [], 1500);
    ...
    if (!defaultAccount) {
      shell.messageEl.textContent = 'No local account found on this device. Open the wallet once online on this device to prime offline export.';
      btn.disabled = true;
      return;
    }
    shell.messageEl.textContent = 'Authenticate with Touch ID/biometrics to export keys offline.';
    btn.disabled = false;
      ...
      try {
        ...
        try {
          ...
          if (isMissingLocalKeyMaterial || isAeadDecryptMismatch) {
            ...
            const challengeB64u = base64UrlEncode(crypto.getRandomValues(new Uint8Array(32)).buffer);
            const allowCredentials = Array.isArray(authenticators) && authenticators.length > 0
              ...
              : [];
            const authCred = await tip.getAuthenticationCredentialsSerializedForChallengeB64u({
    ...
    shell.messageEl.textContent = 'Failed to initialize offline export on this device.';
    btn.disabled = true;
  }
}
