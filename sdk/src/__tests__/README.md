...
- `sdk/src/__tests__/setup/fixtures.ts` extends Playwright `test` with:
...
- Flow helpers in `sdk/src/__tests__/setup/flows.ts`:
...
Example (see `sdk/src/__tests__/setup/flows.ts`):
```ts
