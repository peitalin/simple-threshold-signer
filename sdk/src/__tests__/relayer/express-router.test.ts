...
test.describe('relayer router (express) – P0', () => {
  test('POST /login/options: invalid body', async () => {
    ...
    try {
      const res = await fetchJson(`${srv.baseUrl}/login/options`, {
        method: 'POST',
  ...
  test('POST /login/options: ok returns challengeId', async () => {
    const service = makeFakeAuthService({
  ...
  test('POST /login/verify: invalid body', async () => {
    const service = makeFakeAuthService();
    const router = createRelayRouter(service, {});
    const srv = await startExpressRouter(router);
    try {
      const res = await fetchJson(`${srv.baseUrl}/login/verify`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({}),
      });
  ...
  test('POST /login/verify: not verified maps to 400', async () => {
    const service = makeFakeAuthService({
  ...
  test('POST /login/verify: verified + sessionKind=jwt returns jwt', async () => {
  ...
  test('POST /login/verify: verified + sessionKind=cookie sets Set-Cookie and omits jwt', async () => {
  ...
  test('POST /login/verify: session issuance failures are best-effort', async () => {
    const session = makeSessionAdapter({
