...
test.describe('signer worker – session material waiters', () => {
  ...
  test('fails immediately when SecureConfirm sends {ok:false,error} before the sign request', async ({ page }) => {
    const res = await page.evaluate(async () => {
      ...
      const secureConfirmError = 'SecureConfirm failed (test)';
      channel.port1.postMessage({ ok: false, error: secureConfirmError });
    ...
    expect(res.error).toContain('SecureConfirm failed');
  });
