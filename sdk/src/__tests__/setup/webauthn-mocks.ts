  ...
  const install = () => {
    ...
    if (navigator.credentials) {
      ...
      navigator.credentials.get = async function(options: any) {
        ...
        // **MAIN ISSUE**: WrapKeySeed/session material derivation can fail during login if PRF inputs mismatch
        // **ROOT CAUSE**: Account ID extraction failure during authentication can cause PRF mismatch
        ...
        // - Session material derived with the wrong account hint cannot decrypt previously-encrypted keys
