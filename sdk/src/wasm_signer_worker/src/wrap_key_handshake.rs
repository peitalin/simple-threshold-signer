        ...
        let on_message = move |event: MessageEvent| {
            ...
            if ok == Some(false) {
                let err = js_sys::Reflect::get(&data, &JsValue::from_str("error"))
                    ...
                    .unwrap_or_else(|| "SecureConfirm failed to provide WrapKeySeed".to_string());
            ...
            // One-shot semantics: SecureConfirm sends one message and closes its end; close ours too.
            port_for_close.close();
        };
...
/// the SecureConfirm worker delivers WrapKeySeed over the attached MessagePort.
#[cfg(target_arch = "wasm32")]
