...
export type ThresholdEd25519SessionClaims = {
   ...
   * When present, relayer authorization can validate expiry without a KV record fetch.
   */
  thresholdExpiresAtMs?: number;
  /**
   * Signer-set binding (sorted unique participant ids).
   * When present, relayer authorization can validate signer set without a KV record fetch.
   */
  participantIds?: number[];
...
export function parseThresholdEd25519SessionClaims(raw: unknown): ThresholdEd25519SessionClaims | null {
  ...
  const out: ThresholdEd25519SessionClaims = { sub, kind, sessionId, relayerKeyId, rpId };

  const hasThresholdExpiresAtMs = Object.prototype.hasOwnProperty.call(raw, 'thresholdExpiresAtMs');
  if (hasThresholdExpiresAtMs) {
    ...
    out.thresholdExpiresAtMs = thresholdExpiresAtMs;
  }

  const hasParticipantIds = Object.prototype.hasOwnProperty.call(raw, 'participantIds');
