...
class PostgresThresholdEd25519AuthSessionStore implements ThresholdEd25519AuthSessionStore {
  ...
  async consumeUse(id: string): Promise<ThresholdEd25519AuthConsumeResult> {
    try {
      ...
      const record = parseThresholdEd25519AuthSessionRecord(row.record_json);
      if (!record) return { ok: false, code: 'internal', message: 'Postgres returned invalid auth session record' };
      if (Date.now() > record.expiresAtMs) return { ok: false, code: 'unauthorized', message: 'threshold session expired' };
      return { ok: true, record, remainingUses };
    } catch (e: unknown) {
      const msg = String((e && typeof e === 'object' && 'message' in e) ? (e as { message?: unknown }).message : e || 'Failed to consume threshold session');
      return { ok: false, code: 'internal', message: msg };
