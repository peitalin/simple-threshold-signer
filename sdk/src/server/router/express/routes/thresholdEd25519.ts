    ...
    await handle(ctx, req, res, '/threshold-ed25519/session', {
      ...
      if (!sessionId) {
        return { ok: false, code: 'internal', message: 'threshold session missing sessionId' };
	      ...
	      const userId = String((body as any).sessionPolicy?.nearAccountId || '');
	      const rpId = String((body as any).sessionPolicy?.rpId || '');
	      const relayerKeyId = String((body as any).relayerKeyId || '');
