...
- POST `/login/options` — mint a server-side WebAuthn login challenge (replay-protected). Body:
  - `{ user_id, rp_id, ttl_ms? }` → returns `{ challengeId, challengeB64u, expiresAtMs }`
- POST `/login/verify` — WebAuthn verification + optional session issuance (contract-free). Body:
