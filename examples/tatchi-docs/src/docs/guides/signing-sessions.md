...
- **Warm signing sessions (local)**: Cache a short-lived signing capability in the worker so local signing can happen multiple times without repeated prompts.
...
- **Two prompts at login**: You may be prompting once for backend session issuance and once for a local warm signing session, depending on configuration.
- **Session verification fails**: The login challenge expired or was already used; retry login to mint a fresh challenge.
